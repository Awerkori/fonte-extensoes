...
class WebViewInterceptor(val baseUrl: String, private val userAgent: String?) : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        val isPageApi = url.contains("app.mugiwarasoficial.com/v1/www/works/")
        if ((url.contains("app.mugiwarasoficial.com") && !isPageApi) || url.contains("snipercache.com") || url.contains("/cdn/") || url.endsWith(".jpg") || url.endsWith(".png") || url.endsWith(".webp") || url.endsWith(".jpeg")) {
        ...
        var resultData = fetchViaJs(url, req.method, req.headers, requestBody, isImage, isPageApi)

        if (resultData.result == "HTTP 403") {
            hasErrored = !hasErrored
            resultData = fetchViaJs(url, req.method, req.headers, requestBody, isImage, isPageApi)
        }
                    ...
                    addJavascriptInterface(
                        object {
                            ...
                            fun passResult(reqId: Int, data: String, contentType: String?) {
                                val latch = latches[reqId] ?: return
    ...
    private fun fetchViaJs(
        ...
        mainHandler.post {
            try {
                ...
                webView.webViewClient = object : WebViewClient() {
                    ...
                    override fun onReceivedHttpError(
                        ...
                        if (request.isForMainFrame) {
                            val activeLatch = latches[reqId] ?: return
                    ...
                    override fun onPageFinished(view: WebView, pageUrl: String?) {
                        if (isRsc || isPageApi) {
                            ...
                            (function() {
                                ...
                                const body = contentType.includes('json') && document.body
                                    ? document.body.innerText
                                    : document.documentElement.outerHTML;
                                window.$bridgeName.passResult($reqId, body, contentType);
                            })();
                                """.trimIndent(),
                                null,
                            )
                ...
                val activeLatch = latches[reqId] ?: return@post
                errors[reqId] = e
                activeLatch.countDown()
            }
        }
